...
class LabelEncoder(object):
    ...
    def __call__(self, label):
        ids = self.tokenizer(label.strip(), return_tensors="pt")['input_ids'][0].to(torch.int32)
        return ids
...
@register_task("asr_cif_gpt", dataclass=AudioPretrainingConfig)
class ASR_CIF_GPT_Task(FairseqTask):
    """ """
    ...
    def __init__(
        ...
        self.blank_symbol = "§" #gpt-vocab-100
        self.tokenizer = GPT2Tokenizer.from_pretrained('gpt2')
        self.bos = self.tokenizer.bos_token #"§"  # gpt-vocab-100
        self.pad = "¨"  # gpt-vocab-101
        self.eos = self.tokenizer.eos_token #"©"  # gpt-vocab-102
        self.unk = self.tokenizer.unk_token #"ª"  # gpt-vocab-103
    ...
    def load_target_dictionary(self):
        if self.cfg.labels:
            dict = Dictionary(bos=self.bos,pad=self.pad,eos=self.eos,unk=self.unk,from_tokenizer=True)
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        if task_cfg.labels:
            ...
            with open(label_path, "r") as f:
                labels = [line.lower() for i, line in enumerate(f) if i not in skipped_indices]
            assert len(labels) == len(self.datasets[split]), (
                f"labels length ({len(labels)}) and dataset length "
                f"({len(self.datasets[split])}) do not match"
            )

            process_label = LabelEncoder(self.target_dictionary,self.tokenizer)
