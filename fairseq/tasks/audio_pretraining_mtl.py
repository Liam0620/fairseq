...
class AudioPretrainingConfig(FairseqDataclass):
    ...
    dataset_sampling_alpha: Optional[float] = field(
        default=0.5, metadata={"help": "smoothing alpha for sample rations across datasets"}
    )
...
@register_task("audio_pretraining_mtl", dataclass=AudioPretrainingConfig)
class AudioPretrainingMTLTask(FairseqTask):
    """ """
    ...
    def load_target_dictionary(self):
        if self.cfg.labels:
            dict_path = os.path.join(self.cfg.data,self.dictionary_set, f"dict.{self.cfg.labels}.txt")
    ...
    def dataset_sampler(self,x):
        if self.sample_probs is None:
            # Sample from uniform distribution
            return np.random.choice(x, 1).item()
        else:
            dataset = np.random.choice(x, 1, p=self.sample_probs).item()
            return dataset
    ...
    def _get_sample_prob(self, dataset_lens):
        ...
        smoothed_prob = prob ** self.cfg.dataset_sampling_alpha
    ...
    def load_dataset(self, split: str, task_cfg: FairseqDataclass = None, **kwargs):
        ...
        data_path_list = [os.path.join(data_path_parent,path) for path in os.listdir(data_path_parent)]
        ...
        for data_path in data_path_list:
            ...
            if task_cfg.labels:
                ...
                if os.path.exists(label_path):
                    skipped_indices = getattr(dataset_map[data_path], "skipped_indices", set())
            ...
            datasets_lengths.append(sum(dataset_map[data_path].sizes) / task_cfg.sample_rate / 3600)

        datasets_lengths = np.array(datasets_lengths)
        self.sample_probs = self._get_sample_prob(datasets_lengths)
        size_ratio = (self.sample_probs * datasets_lengths.sum()) / datasets_lengths
        ...
        self.datasets[split] = MultiCorpusSampledDataset(dataset_map,sampling_func=self.dataset_sampler)
